...
<p align="center">
  <strong>一个可嵌入的高性能用户态网络库，内部实现了可编程协议栈 — 用 C++20 从零实现。</strong><br>
  <sub>面向那些更看重传输层行为、可观测性与包路径控制，而不是通用操作系统兼容性的系统场景。</sub>
</p>
...
NeuStack 是一个**可嵌入的用户态网络库**，用 C++20 编写。这个库内部实现了一套可编程协议栈，覆盖从 Ethernet / IPv4 到 TCP/UDP、HTTP、DNS、Telemetry，以及可选的 AI 辅助传输组件。

这个项目主要面向这样一类环境：网络栈本身就是系统设计的一部分。你可能想主动控制拥塞行为、实时观测传输状态、对比不同数据路径，或者把特定领域逻辑直接挂在包处理链路上。

**为什么选择用户态？** 内核网络栈首先是为通用兼容性而设计的，而不是为了让你自由改写传输语义。把协议栈放到用户空间，可以获得：
...
因此，NeuStack 很适合 **分布式 AI 训练**、**推理服务**、**HPC 通信**、**低延迟服务**、**系统研究** 这类场景。它们只是例子，不是边界；更本质的共同点是：网络行为本身就是业务或基础设施设计的一部分，而不只是默认背景设施。

NeuStack **并不打算** 成为桌面或移动端操作系统网络栈的直接替代品。更准确的定位是：一个面向专用服务、受控环境、性能实验和传输研究的可编程协议栈库。
...
- **深入可观测性**：Prometheus/JSON 遥测、7 个实时 HTTP 端点、Web Dashboard、`neustack-stat` 实时 CLI
- **统一跨平台 HAL**：macOS utun · Linux TUN/TAP + AF_XDP · Windows Wintun — 统一 API

## 项目定位

NeuStack 当前最擅长的是那些你能控制环境、也能控制流量路径的场景：

- **协议与系统研究**：快速实验传输行为、包处理策略、指标体系和故障语义，不必反复进入内核开发循环
- **专用基础设施路径**：构建那些需要让传输策略、拥塞控制或 Telemetry 与应用语义联动的服务
- **性能验证与数据面实验**：在可复现负载下评估用户态快路径、AF_XDP 后端和自定义传输逻辑

同样重要的是，NeuStack **不** 把自己定位成：

- 面向普通消费者的“网络加速器”
- 以浏览器上网为第一目标的桌面网络替代方案
- 一个已经覆盖成熟操作系统网络栈全部兼容性的成品

这不是保守，而是刻意选择。NeuStack 优先优化的是 **可编程性、可观测性和专用性能路径**，而不是假设所有网络工作负载都追求同一种“通用更快”。
...
>
> 这些数字更适合被理解为“快路径已经成立”的证据，而不是“NeuStack 可以无条件替代普通终端上的通用互联网协议栈”的宣称。
